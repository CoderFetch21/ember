#!/usr/bin/env python3
# ============================================================
#  EMBER BASE - Standalone Base Model
#  A language model built from scratch.
#
#  Usage:
#    python ember-base.py
#    python ember-base.py --checkpoint path/to/checkpoint.pt
#
#  Requirements: torch
# ============================================================
...
def generate(model, tokenizer, prompt: str, max_new_tokens: int = 200,
             temperature: float = 0.8, top_k: int = 50, top_p: float = 0.95) -> str:
    model.eval()
...
def load_model(checkpoint_path: str, tokenizer_path: str):
    ...
    print(f"  ✅ Ember Base loaded ({total:,} parameters)")
    return model, tokenizer

# ── Chat Interface ─────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="Ember Base Model")
...
╚══════════════════════════════════════════╝
    ...
    print("\nEmber Base is ready. Enter a prompt to continue.\n")
    print("─" * 50)
    ...
    while True:
        ...
        if prompt.lower().startswith("set "):
            ...
            if len(parts) == 3 and parts[1] in settings:
                try:
                    settings[parts[1]] = type(settings[parts[1]])(parts[2])
